...
```bash
docker compose -f docker-compose.yml run --rm django python manage.py makemessages -all --no-location
```
...
```bash
docker compose -f docker-compose.yml run --rm django python manage.py compilemessages
```
